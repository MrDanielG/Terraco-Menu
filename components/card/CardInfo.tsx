...
const CardInfoTitle = (props: any) => {
    return (
        <h1 className="block text-md leading-tight text-black hover:underline">{props.children}</h1>
    );
};

const CardInfoBody = (props: any) => {
    return (
        <div className="uppercase mt-2 tracking-wide text-sm text-gray-400">{props.children}</div>
    );
};
...
const CardInfo: React.FC<Props> & {
    ...
    return <div className="p-5 w-full flex flex-col justify-center">{props.children}</div>;
};
